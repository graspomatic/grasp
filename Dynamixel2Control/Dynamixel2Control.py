...
class D2C(object):
    def __init__(self):
        ...
        # -------------------------------------
        dxl_comm_result, dxl_error = self.packet_handler.write1ByteTxRx(self.port_handler, DXL_ID, dxlcx.ADDR_OPERATING_MODE, 3)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % self.packet_handler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % self.packet_handler.getRxPacketError(dxl_error))
        ...
        # (Lower P means lower response near goal)
        # -------------------------------------
        P_GAIN = 100   # typical range 0~16000 in some firmware
        I_GAIN = 0
        D_GAIN = 0
        dxl_comm_result, dxl_error = self.packet_handler.write2ByteTxRx(self.port_handler, DXL_ID, dxlcx.ADDR_POSITION_P_GAIN, P_GAIN)
        dxl_comm_result, dxl_error = self.packet_handler.write2ByteTxRx(self.port_handler, DXL_ID, dxlcx.ADDR_POSITION_I_GAIN, I_GAIN)
        dxl_comm_result, dxl_error = self.packet_handler.write2ByteTxRx(self.port_handler, DXL_ID, dxlcx.ADDR_POSITION_D_GAIN, D_GAIN)
        
        # -------------------------------------
        # Set a "PWM Limit" so the servo won't
        # apply maximum torque.
        # (Range is typically 0 ~ 885 for MX servos.)
        # -------------------------------------
        PWM_LIMIT = 200  # try 200 ~ 300 as a start
        dxl_comm_result, dxl_error = self.packet_handler.write2ByteTxRx(self.port_handler, DXL_ID, dxlcx.ADDR_PWM_LIMIT, PWM_LIMIT)
        
        # -------------------------------------
        # "Moving Threshold" controls how big
        # a velocity must be to consider the servo "moving."
        # This does NOT truly create a torque deadband,
        # but if set high, small motions won't flag it as 'moving.'
        # -------------------------------------
        MOVING_THRESHOLD = 50
        dxl_comm_result, dxl_error = self.packet_handler.write4ByteTxRx(self.port_handler, DXL_ID, dxlcx.ADDR_MOVING_THRESH, MOVING_THRESHOLD)
        
        # -------------------------------------
        # Re-enable torque
        # -------------------------------------
        dxl_comm_result, dxl_error = self.packet_handler.write1ByteTxRx(self.port_handler, DXL_ID, dxlcx.ADDR_TORQUE_ENABLE, 1)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % self.packet_handler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % self.packet_handler.getRxPacketError(dxl_error))
        ...
        # -------------------------------------
        GOAL_POSITION = 2000
        dxl_comm_result, dxl_error = self.packet_handler.write4ByteTxRx(self.port_handler, DXL_ID, dxlcx.ADDR_GOAL_POSITION, GOAL_POSITION)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % self.packet_handler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % self.packet_handler.getRxPacketError(dxl_error))
