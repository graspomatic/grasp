...
async def find_bounds(axis = ['a'], direction = [-1]):
    ...
    if axis == 'x':
        ...
        if direction == 1:
            await y.find_bound(direction, current=0.15)
        else:
            await y.find_bound(direction, current=0.9)
...
async def toggle_touch(side, status):
    ...
    if status:
        sock.sendall(b'%set sensor:control:activate %i' % side)
    else:
        sock.sendall(b'%set sensor:control:deactivate %i' % side)
    b = sock.recv(128)
    print(b)
