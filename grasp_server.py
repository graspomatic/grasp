...
async def return_object(side=-1, add=[0,0]):
    ...
    await loop.create_task(wait_for_dxl(150))  # 190 seems too high, 180 once dropped off way too early
...
async def retrieve(side=-1, objid=0, add=[0,0]):
    ...
    await loop.create_task(wait_for_dxl(205))  # was 190, trying to improve occasional failed pickup. maybe
    await loop.create_task(mags.energize(side))
    await asyncio.sleep(0.25)  # need to wait a bit for the magnet to suck in the object. was 0.1
    if side == 0:
...
async def pick_and_place(hand=[-1], left_id=[-1], right_id=[-1], left_angle=[180], right_angle=[180], return_duplicates=[1], dont_present=[-1], xoffset=[0]):
